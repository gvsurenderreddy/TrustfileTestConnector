...
class ErrorsControllerTest < ActionController::TestCase
  ...
  test "should create error" do
    assert_difference('Error.count') do
      post :create, error: { message: @error.message, timestamp: @error.timestamp, type: @error.type }
    end
  ...
  test "should update error" do
    patch :update, id: @error, error: { message: @error.message, timestamp: @error.timestamp, type: @error.type }
    assert_redirected_to error_path(assigns(:error))
  end
